...
func EncryptMessage(
	...
	data := append(encryptedAESKey, encryptedPayload...)
	signature := ed25519.Sign(senderEDPrivate, data)

	return append(data, signature...), nil
}

func DecryptMessage(data []byte, receiverECDH *ecdh.PrivateKey, senderECPub *ecdh.PublicKey, senderEDPub ed25519.PublicKey) ([]byte, error) {
	if len(data) < 64 {
		return nil, fmt.Errorf("data too short: missing signature")
	}
